...
const BannerItem = ({ item }) => {
  const { data, error } = useSWR(
    `https://api.themoviedb.org/3/movie/${item.id}/credits?api_key=8b3c7ef30b0460b1ce585fb7c8f7c4c0`,
    fetcher
  );
  if (!data) return null;
  const { cast, profile_path } = data;
  console.log(cast);
  const { backdrop_path, original_title, release_date, overview } = item;
      ...
      <div className="absolute w-full banner-info left-16 top-[7.25rem] ">
        ...
        <div className="flex items-center justify-between w-[1024px]">
          <div className="">
            ...
            <p className="text-sm text-gray-400 max-w-[300px]">{overview}</p>
            <div className="cast">
                  ...
                  cast.slice(0, 5).map((item) => (
                    <div className="flex items-center">
                      <img
                        ...
                        src={`https://image.tmdb.org/t/p/original/${profile_path}&&api_key=8b3c7ef30b0460b1ce585fb7c8f7c4c0`}
                        alt=""
                      />
        ...
        <div className="flex items-center w-full mt-6">
          ...
          <div>
            <Button
              className={
                " hover:bg-gray-400 hover:text-black relative left-[570px]"
              }
            >
              Like
            </Button>
