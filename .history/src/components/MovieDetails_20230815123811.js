...
const MovieInfo = ({ itemData }) => {
  console.log(itemData);
  const { poster_path, genres, overview, release_date, title, vote_average } =
    data || [];
  const { data, error } = useSWR(
    `https://api.themoviedb.org/3/movie/298618/credits?api_key=8b3c7ef30b0460b1ce585fb7c8f7c4c0`,
    fetcher
  );
  if (!data) return null;
  const { crew } = data || [];
  ...
  return (
    <div className="movie_detail flex max-w-[1280px]">
      <div className="flex movie_info bg-[#222222] px-3 py-3 mt-5 ">
        ...
        <div className="ml-4 text-white">
          <h2 className="mb-4 text-3xl text-red-400 mr-2">
            Nhiệm vụ bất khả khi
          </h2>
  ...
  return (
    <div className="bg-[#282828] max-w-[630px] mt-8 p-3  ">
      ...
      <p className=" max-w-[600px] text-xs text-gray-400">
        Nhịp Đập Trái Tim kể về một ma cà rồng bán người tên là Seon Woo-hyeol,
        người đã không thể trở thành con người vào một ngày nào đó trong vòng
        100 năm, và một người phụ nữ Joo In-hae, người không có một chút lễ nghi
        nào của con người, bắt đầu sống bên nhau và tìm thấy hơi ấm thật sự.
      </p>
    </div>
  );
};
const MovieDetails = () => {
  const { data, error } = useSWR(
    `https://api.themoviedb.org/3/movie/298618?api_key=8b3c7ef30b0460b1ce585fb7c8f7c4c0`,
    fetcher
  );
  if (!data) return null;
  ...
  return (
    <div className="relative z-50 top-[55px] h-auto pl-[160px] ">
      <MovieInfo data={itemData}></MovieInfo>
      <MovieDecs></MovieDecs>
    </div>
  );
};
