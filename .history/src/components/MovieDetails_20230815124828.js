...
const MovieInfo = ({ dataItem }) => {
  ...
  const { data, error } = useSWR(
    `https://api.themoviedb.org/3/movie/${idMovie.movieID}/credits?api_key=8b3c7ef30b0460b1ce585fb7c8f7c4c0`,
    fetcher
  );
  if (!data) return null;
  const { crew } = data || [];
...
const MovieDetails = () => {
  ...
  console.log(idMovie);
  const { data, error } = useSWR(
    `https://api.themoviedb.org/3/movie/${idMovie}?api_key=8b3c7ef30b0460b1ce585fb7c8f7c4c0`,
    fetcher
  );
  if (!data) return null;
  const itemData = data;
