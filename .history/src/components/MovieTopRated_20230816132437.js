...
const MovieCardItem = ({ item }) => {
  ...
  return (
    ...
    >
      <img
        className="rounded-xl w-[200px] h-[150px] hover:scale-[1.07] ease-linear duration-200 object-cover mb-3"
        src={`https://image.tmdb.org/t/p/original${poster_path}`}
        alt=""
      />
      <p className="my-3 text-sm font-bold text-white max-w-[200px] truncate mt-2 mb-1 ">
        {title}
      </p>
...
const MovieTopRated = () => {
  const { data, error } = useSWR(
    `https://api.themoviedb.org/3/movie/top_rated?api_key=8b3c7ef30b0460b1ce585fb7c8f7c4c0`,
    fetcher
  );
  if (!data) return null;
  const { results } = data;
